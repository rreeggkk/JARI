...
public class TileEntityHydraulicSeparator extends TileEnergyHandler implements ISidedInventory, IFluidHandler{
	...
	public int getFlameScaled(int maximum) {
		return (int)((((float)getCurrentEnergyPerTick()*maximum / (float)getMaxEnergyPerTick())));
	}
	@SideOnly(Side.CLIENT)
	public int getWaterScaled(int maximum) {
		return (int)((((float)tank.getFluidAmount()*maximum / (float)maxWater)));
	}
	...
	public void setWaterCount(int i) {
		tank.fill(new FluidStack(FluidRegistry.WATER, i - tank.getFluidAmount()), true);
	}
