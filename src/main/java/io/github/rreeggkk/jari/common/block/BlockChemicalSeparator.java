...
/**
 *
 * The block for the Chemical Separator. It is based of of the Vanilla furnace.
 *
 * @author rreeggkk
 *
 */
public class BlockChemicalSeparator extends BlockContainer {
	...
	/**
	 * The construtor for the Chemical Separator class
	 */
	public BlockChemicalSeparator() {
		...
		// Set the block name
		setBlockName("blockChemicalSeparator");
	}
	...
	public boolean onBlockActivated(World world, int x, int y, int z,
		...
		// Open the GUI
		player.openGui(JARI.instance, GuiIDs.CHEMICAL_SEPARATOR, world, x, y,
				z);
	...
	public void breakBlock(World world, int x, int y, int z, Block block,
			int p_149749_6_) {
		// Get the tile entity
		TileEntityChemicalSeparator tile = (TileEntityChemicalSeparator) world
				.getTileEntity(x, y, z);
	...
	public void randomDisplayTick(World world, int x, int y, int z,
		...
		if (isOn(world, x, y, z)) {
			...
			// Spawn crit particles on front face of machine
			if (l == 9) {
				world.spawnParticle("crit", f - f3, f1, f2 + f4, 0.0D, 0.0D,
						0.0D);
				world.spawnParticle("crit", f - f3, f1, f2 + f4, 0.0D, 0.0D,
						0.0D);
			} else if (l == 11) {
				...
				world.spawnParticle("crit", f + f4, f1, f2 - f3, 0.0D, 0.0D,
						0.0D);
				world.spawnParticle("crit", f + f4, f1, f2 - f3, 0.0D, 0.0D,
						0.0D);
			} else if (l == 7) {
	...
	public int getComparatorInputOverride(World world, int x, int y, int z,
			int p_149736_5_) {
		return ((TileEntityChemicalSeparator) world.getTileEntity(x, y, z))
				.getComparatorOutput();
	}
