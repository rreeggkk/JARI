...
/**
 *
 * The block for the Centrifuge. It is based of of the Vanilla furnace.
 *
 * @author rreeggkk
 *
 */
public class BlockCentrifuge extends BlockContainer {
	...
	private IIcon onTexture, bottomTexture, topTexture;

	/**
	 * The construtor for the Centrifuge class
	 */
	...
	public IIcon getIcon(int side, int meta) {
		...
		return side == 1 ? topTexture : side == 0 ?
				bottomTexture : isOn(meta) ? onTexture : bottomTexture;
	}
	...
	public void registerBlockIcons(IIconRegister iconRegistry) {
		...
		onTexture = iconRegistry.registerIcon(ModInformation.ID
				+ ":centrifuge_side_on");
		bottomTexture = iconRegistry.registerIcon(ModInformation.ID
				+ ":centrifuge_bottom");
	...
	public boolean onBlockActivated(World world, int x, int y, int z,
		...
		// Open the GUI
		player.openGui(JARI.instance, GuiIDs.CENTRIFUGE, world, x, y,
				z);
	...
	public void breakBlock(World world, int x, int y, int z, Block block,
			int p_149749_6_) {
		// Get the tile entity
		TileEntityCentrifuge tile = (TileEntityCentrifuge) world
				.getTileEntity(x, y, z);
	...
	public int getComparatorInputOverride(World world, int x, int y, int z,
			int p_149736_5_) {
		return ((TileEntityCentrifuge) world.getTileEntity(x, y, z))
				.getComparatorOutput();
	}
