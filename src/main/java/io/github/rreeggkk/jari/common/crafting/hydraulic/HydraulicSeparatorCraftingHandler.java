...
public class HydraulicSeparatorCraftingHandler {
	...
	static {
		instance.addRecipe(new ItemStack(Blocks.log), new ItemStack(Blocks.planks), 50000, 16000);
		...
		instance.addRecipe(new OreHydraulicRecipe("oreLapis", 1, new ItemStack(Items.dye, 18, 4), 5000, 1000));
		instance.addRecipe(new OreHydraulicRecipe("oreDiamond", 1, new ItemStack(Items.diamond, 3), 10000, 4000));
		instance.addRecipe("oreRedstone", "dustRedstone", 18, 5000, 1000);
		instance.addRecipe(new OreHydraulicRecipe("oreEmerald", 1, new ItemStack(Items.emerald, 3), 10000, 4000));
		instance.addRecipe(new OreHydraulicRecipe("oreNetherQuartz", 1, new ItemStack(Items.quartz, 5), 4000, 800));
	 ...
	 * @param input ItemStack (Will use Item type and quantity)
	 * @param output ItemStack (Will use Item type and quantity)
	 * @param energy required amount of energy
	 * @param water required amount of water
	 */
	public void addRecipe(ItemStack input, ItemStack output, int energy, int water){
		recipes.add(new HydraulicRecipe(input, output, energy, water));
	}
	/**
	 * Adds an OreDictionary recipe
	 * @param input String OreDictionary entry
	 * @param output String OreDictionary entry
	 * @param energy required amount of energy
	 * @param water required amount of water
	 */
	 ...
	 * @param input String OreDictionary entry
	 * @param output String OreDictionary entry
	 * @param outputNum number of items for output
	 * @param energy required amount of energy
	 * @param water required amount of water
	 */
	public void addRecipe(String input, String output, int outputNum, int energy, int water){
		recipes.add(new OreHydraulicRecipe(input, output, outputNum, energy, water));
	}
	/**
	 * Adds an OreDictionary recipe
	 * @param input String OreDictionary entry
	 * @param inputNum number of items for input
	 * @param output String OreDictionary entry
	 * @param outputNum number of items for output
	 * @param energy required amount of energy
	 * @param water required amount of water
	 */
	public void addRecipe(String input, int inputNum, String output, int outputNum, int energy, int water){
		recipes.add(new OreHydraulicRecipe(input, inputNum, output, outputNum, energy, water));
	}
	/**
	 * Add a custom recipe
	 ...
	 * @param input ItemStack input for recipe
