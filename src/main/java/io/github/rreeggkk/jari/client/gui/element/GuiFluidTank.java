...
public class GuiFluidTank extends GuiRectangle {
	...
	public void drawBackgroundLayer(float f1, float f2, float f3) {
		...
		gui.drawTexturedModalRect(gui.getGuiLeft() + x, gui.getGuiTop() + y, 0,
				0, 18, 73);

		if (fluid != null && fluid.getFluid() != null) {
			int waterSize = (int) ((float) fluid.getFluid().amount * 71 / fluid
					.getCapacity());
			drawFluid(fluid.getFluid(), gui.getGuiLeft() + x + 1,
					gui.getGuiTop() + y + 1 + 71 - waterSize, 16, waterSize, 71);
